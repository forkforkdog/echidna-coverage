...
function parseFunctions(lines: string[], allFunctions: boolean): FunctionBlock[] {
  ...
  const contractLevel: FunctionBlock = {
    name: "<contract-level>",
    lines: [],
    isTouched: false,
    isReverted: false,
  ...
  lines.forEach((line, index) => {
    ...
    const constructorMatch = !functionMatch ? content.match(
      /\b(constructor|modifier\s+([a-zA-Z_][a-zA-Z0-9_]*))\s*\(/
    ) : null;
    const receiveMatch = (!functionMatch && !constructorMatch) ? content.match(
      /\b(receive|fallback)\s*\(\s*\)/
    ) : null;
